	...
	private_vici_cred_t *this, char *name, u_int id, vici_message_t *message)
{
	...
	hex = message->get_str(message, NULL, "cka_id");
	if (!hex)
	{
		return create_reply("keyid missing");
	}
	cka_id = chunk_from_hex(chunk_from_str(hex), NULL);
	...
	{	/* provide the pin in a temporary credential set to access the key */
		...
		owner = identification_create_from_encoding(ID_KEY_ID, cka_id);
		set = mem_cred_create();
